...
describe("CLI", () => {
    ...
    */
    ...
    it("release-react uses ram-bundle", (done: MochaDone): void => {
        var bundleName = "bundle.js";
        var command: cli.IReleaseReactCommand = {
            type: cli.CommandType.releaseReact,
            appName: "a",
        ...
        cmdexec.execute(command)
            .then(() => {
                ...
                assert.equal(
                    spawnCommandArgs,
                    `${path.join("node_modules", "react-native", "local-cli", "cli.js")} ram-bundle --assets-dest ${path.join(os.tmpdir(), "CodePush/CodePush")} --bundle-output ${path.join(os.tmpdir(), "CodePush/CodePush", bundleName)} --dev false --entry-file index.android.js --platform android --config /path/to/config.json`
                );
                assertJsonDescribesObject(JSON.stringify(release.args[0][0], /*replacer=*/ null, /*spacing=*/ 2), releaseCommand);
                done();
            })
    ...
    it("release-react uses indexed-ram-bundle", (done: MochaDone): void => {
        var bundleName = "bundle.js";
        var command: cli.IReleaseReactCommand = {
            type: cli.CommandType.releaseReact,
            appName: "a",
        ...
        cmdexec.execute(command)
            .then(() => {
                ...
                assert.equal(
                    spawnCommandArgs,
                    `${path.join("node_modules", "react-native", "local-cli", "cli.js")} ram-bundle --assets-dest ${path.join(os.tmpdir(), "CodePush/CodePush")} --bundle-output ${path.join(os.tmpdir(), "CodePush/CodePush", bundleName)} --dev false --entry-file index.android.js --platform android --config /path/to/config.json --indexed-ram-bundle`
                );
                assertJsonDescribesObject(JSON.stringify(release.args[0][0], /*replacer=*/ null, /*spacing=*/ 2), releaseCommand);
                done();
            })
    ...
    it("release-react fails if indexedRamBundle is used without bundleCommand ram-bundle", (done: MochaDone): void => {
        var bundleName = "bundle.js";
        var command: cli.IReleaseReactCommand = {
            type: cli.CommandType.releaseReact,
            appName: "a",
        ...
        cmdexec.execute(command)
            ...
            .catch((err) => {
                assert.equal(err.message, "Indexed RAM bundle must be used with \"--bundleCommand ram-bundle\"");
                sinon.assert.notCalled(release);
                sinon.assert.notCalled(spawn);
                done();
            })
