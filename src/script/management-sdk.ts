...
class AccountManager {
  ...
  constructor(
    ...
    if (!accessKey)
      throw new CodePushUnauthorizedError("A token must be specified.");
  ...
  public isAuthenticated(throwIfUnauthorized?: boolean): Promise<boolean> {
    return new Promise<any>((resolve, reject) => {
      var request: superagent.Request = superagent.get(
        this._serverUrl + urlEncode`/authenticated`
      );
  ...
  public addAccessKey(friendlyName: string, ttl?: number): Promise<AccessKey> {
    if (!friendlyName) {
      throw new CodePushUnauthorizedError(
        "A name must be specified when adding an access key."
      );
    }
    ...
    return this.post(
      urlEncode`/accessKeys/`,
      JSON.stringify(accessKeyRequest),
  ...
  public getAccessKey(accessKeyName: string): Promise<AccessKey> {
    return this.get(urlEncode`/accessKeys/${accessKeyName}`).then(
      (res: JsonResponse) => {
        return {
  ...
  public getSessions(): Promise<Session[]> {
    return this.get(urlEncode`/accessKeys`).then((res: JsonResponse) => {
      ...
      var sessions: Session[] = Object.keys(sessionMap).map(
        (machineName: string) => sessionMap[machineName]
      );
  ...
  public getAccountInfo(): Promise<Account> {
    return this.get(urlEncode`/account`).then(
      (res: JsonResponse) => res.body.account
    );
  }

  // Apps
  public getApps(): Promise<App[]> {
    return this.get(urlEncode`/apps`).then(
      (res: JsonResponse) => res.body.apps
    );
  }

  public getApp(appName: string): Promise<App> {
    return this.get(urlEncode`/apps/${this.appNameParam(appName)}`).then(
      (res: JsonResponse) => res.body.app
    );
  }
  ...
  public addApp(
    ...
    appOs: string,
    appPlatform: string,
    manuallyProvisionDeployments: boolean = false
  ): Promise<App> {
  ...
  public removeApp(appName: string): Promise<void> {
    return this.del(urlEncode`/apps/${this.appNameParam(appName)}`).then(
      () => null
    );
  }
  ...
  public renameApp(oldAppName: string, newAppName: string): Promise<void> {
    return this.patch(
      urlEncode`/apps/${this.appNameParam(oldAppName)}`,
      JSON.stringify({ name: newAppName })
    ).then(() => null);
  }

  public transferApp(appName: string, email: string): Promise<void> {
    return this.post(
      urlEncode`/apps/${this.appNameParam(appName)}/transfer/${email}`,
      /*requestBody=*/ null,
      /*expectResponseBody=*/ false
    ).then(() => null);
  }
  ...
  public getCollaborators(appName: string): Promise<CollaboratorMap> {
    return this.get(
      urlEncode`/apps/${this.appNameParam(appName)}/collaborators`
    ).then((res: JsonResponse) => res.body.collaborators);
  }

  public addCollaborator(appName: string, email: string): Promise<void> {
    return this.post(
      urlEncode`/apps/${this.appNameParam(appName)}/collaborators/${email}`,
      /*requestBody=*/ null,
      /*expectResponseBody=*/ false
    ).then(() => null);
  }
  ...
  public addDeployment(
    ...
    return this.post(
      urlEncode`/apps/${this.appNameParam(appName)}/deployments/`,
      JSON.stringify(deployment),
      /*expectResponseBody=*/ true
    ).then((res: JsonResponse) => res.body.deployment);
  }

  public clearDeploymentHistory(
    appName: string,
    deploymentName: string
  ): Promise<void> {
    return this.del(
      urlEncode`/apps/${this.appNameParam(
        appName
      )}/deployments/${deploymentName}/history`
    ).then(() => null);
  }
  ...
  public getDeploymentMetrics(
    ...
    return this.get(
      urlEncode`/apps/${this.appNameParam(
        appName
      )}/deployments/${deploymentName}/metrics`
  ...
  public release(
    ...
    return new Promise<Package>((resolve, reject) => {
      ...
      var request: superagent.Request = superagent.post(
        this._serverUrl +
          urlEncode`/apps/${this.appNameParam(
            appName
      ...
      var getPackageFilePromise: Promise<PackageFile> = this.packageFileFromPath(
        filePath
      );
  ...
  public promote(
    ...
    return this.post(
      urlEncode`/apps/${this.appNameParam(
        appName
      )}/deployments/${sourceDeploymentName}/promote/${destinationDeploymentName}`,
      requestBody,
      /*expectResponseBody=*/ true
  ...
  public rollback(
    ...
    return this.post(
      urlEncode`/apps/${this.appNameParam(
        appName
      )}/deployments/${deploymentName}/rollback/${targetRelease || ``}`,
      /*requestBody=*/ null,
      /*expectResponseBody=*/ false
    ).then(() => null);
  }
  ...
  private packageFileFromPath(filePath: string): Promise<PackageFile> {
    ...
    if (fs.lstatSync(filePath).isDirectory()) {
      getPackageFilePromise = new Promise<PackageFile>(
        (
          resolve: (file: PackageFile) => void,
  ...
  private makeApiRequest(
    ...
    return new Promise<JsonResponse>((resolve, reject) => {
      var request: superagent.Request = (<any>superagent)[method](
        this._serverUrl + endpoint
      );
      if (this._proxy) (<any>request).proxy(this._proxy);
      this.attachCredentials(request);
  ...
  private getErrorStatus(error: any, response: superagent.Response): number {
    return (
      ...
      AccountManager.ERROR_GATEWAY_TIMEOUT
    );
  }
  ...
  private attachCredentials(request: superagent.Request): void {
    ...
    request.set(
      "Accept",
      `application/vnd.code-push.v${AccountManager.API_VERSION}+json`
    );
